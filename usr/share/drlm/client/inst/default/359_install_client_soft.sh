	...
	case $RELEASE in
		[6*-7*])
			...
			then 
				...
				ssh -t root@$CLI_NAME ' if [ ! -d /tmp/DRLM ]; then mkdir /tmp/DRLM/; else rm -rf /tmp/DRLM/*.deb; fi'
				scp ${DEPDIR}/${DISTRO}/${VERSION}/$RELEASE/*.deb root@$CLI_NAME:/tmp/DRLM/. 
				ssh -t root@$CLI_NAME ' for line in $(ls /tmp/DRLM/*.deb); do dpkg -i $line; done'
			fi
			send_config_rear $CLI_NAME
			ssh root@${CLI_NAME} "ex -s -c ':/kernel/s/\$PXE_KERNEL/\$DRLM_NAME\/\$OUTPUT_PREFIX\/\$PXE_KERNEL/g' -c ':wq' /usr/share/rear/output/PXE/default/81_create_pxelinux_cfg.sh"
			ssh root@${CLI_NAME} "ex -s -c ':/append/s/\$PXE_INITRD/\$DRLM_NAME\/\$OUTPUT_PREFIX\/\$PXE_INITRD/g' -c ':wq' /usr/share/rear/output/PXE/default/81_create_pxelinux_cfg.sh"
			if config_sudo; then send_sudo_config ${CLI_NAME}; fi
			ssh-copy-id $DRLM_USER@${CLI_NAME}
		#	ssh root@${CLI_NAME} 'passwd -l $DRLM_USER'
	...
	case $RELEASE in
		[5*-6*])
			...
			then
				...
				ssh -t root@$CLI_NAME ' if [ ! -d /tmp/DRLM ]; then mkdir /tmp/DRLM/; else rm -rf /tmp/DRLM/*.rpm; fi'
				scp ${DEPDIR}/${DISTRO}/${VERSION}/$RELEASE/*.rpm root@$CLI_NAME:/tmp/DRLM/.
				ssh -t root@$CLI_NAME ' for line in $(ls /tmp/DRLM/*.rpm); do rpm -Uvh $line; done'
			fi
			ssh -t root@$CLI_NAME ' rm -rf /tmp/DRLM/*.rpm'
			send_config_rear $CLI_NAME
			ssh root@${CLI_NAME} "ex -s -c ':/kernel/s/\$PXE_KERNEL/\$DRLM_NAME\/\$OUTPUT_PREFIX\/\$PXE_KERNEL/g' -c ':wq' /usr/share/rear/output/PXE/default/81_create_pxelinux_cfg.sh"
			ssh root@${CLI_NAME} "ex -s -c ':/append/s/\$PXE_INITRD/\$DRLM_NAME\/\$OUTPUT_PREFIX\/\$PXE_INITRD/g' -c ':wq' /usr/share/rear/output/PXE/default/81_create_pxelinux_cfg.sh"
			if config_sudo; then send_sudo_config ${CLI_NAME}; fi
