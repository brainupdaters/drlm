...
Log "                                                                  "
Log " - Modifying DR Network $NET_ID$NET_NAME from DRLM ...            "
Log "                                                                  "
...
if test -n "$NET_NAME"; then
        ...
        then
                ...
                Log "$PROGRAM:$WORKFLOW: ${NET_NAME} found in DRLM database!"
        else
                #report_error "$PROGRAM: Network named: $NET_NAME not registered!"
                Error "$PROGRAM:$WORKFLOW: Network named: $NET_NAME not registered!"
        fi
else
        ...
        then
                ...
                Log "$PROGRAM:$WORKFLOW: ${NET_ID} found in DRLM database!"
        else
                #report_error "$PROGRAM: Network with ID: $NET_ID not registered!"
                Error "$PROGRAM:$WORKFLOW: Network with ID: $NET_ID not registered!"
        fi
