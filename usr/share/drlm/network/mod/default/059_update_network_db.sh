...
if test -n "$NET_SRV"; then
	...
	if mod_network_srv "$NET_ID" "$NET_SRV" ;
        then
...
if test -n "$NET_GW"; then
	...
	if mod_network_gw "$NET_ID" "$NET_GW" ;
        then
...
if test -n "$NET_MASK"; then
	...
	if mod_network_mask "$NET_ID" "$NET_MASK" ;
        then
...
if mod_network_bcast "$NET_ID" "$NET_BCAST" ;
then
...
if mod_network_ip "$NET_ID" "$NET_IP" ;
then
...
#if mod_network_ "$NET_ID" "$NET_" ;
#then
#	Log "${NET_NAME} modified in the database!"
#else
#	Error "$PROGRAM: $NET_NAME not modified!"
#fi
