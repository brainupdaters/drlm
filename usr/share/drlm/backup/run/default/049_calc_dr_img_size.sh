...
INCLUDE_LIST=( ${ONLY_INCLUDE_VG[@]} ${INCLUDE_VG[@]} )

EXCLUDE_LIST=( ${EXCLUDE_MOUNTPOINTS[@]} ${EXCLUDE_VG[@]} ${BACKUP_PROG_EXCLUDE[@]} )

if DR_IMG_SIZE_MB=$(ssh -tt $DRLM_USER@$CLI_NAME "$(declare -p INCLUDE_LIST EXCLUDE_LIST ; declare -f get_fs_free_mb get_fs_size_mb get_fs_used_mb get_client_used_mb); get_client_used_mb") ;
then
