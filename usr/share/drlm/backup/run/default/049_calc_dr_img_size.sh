...
if DR_IMG_SIZE_MB=$(ssh $SSH_OPTS $DRLM_USER@$CLI_NAME "$(declare -p EXCLUDE_LIST INCLUDE_LIST_VG EXCLUDE_LIST_VG ; declare -f get_fs_free_mb get_fs_size_mb get_fs_used_mb get_client_used_mb); get_client_used_mb" 2>/dev/null) ; then
    Log "$PROGRAM:$WORKFLOW: Remote space collection returned $DR_IMG_SIZE_MB MB for $CLI_NAME backup."
    let "DR_IMG_SIZE_MB+=DR_IMG_SIZE_MB*10/100"
else
    Error "$PROGRAM:$WORKFLOW: Problem collecting remote space! aborting ..."
fi

# Check if returned value is a number
re='^[0-9]+$'
if ! [[ $DR_IMG_SIZE_MB =~ $re ]] ; then
   Error "$PROGRAM:$WORKFLOW: Problem collecting remote space! DR_IMG_SIZE_MB is not numeric. aborting ..."
fi
