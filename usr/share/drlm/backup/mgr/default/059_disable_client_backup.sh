...
function wf_disable_client_backup(){
   ...
   if [ -n "$A_BKP_ID_DB" ] ; then
      ...
      LOOP_DEVICE=$(losetup --list | grep -w "$A_BKP_ID_DB" | awk '{print $1}')

      Log "$PROGRAM:$WORKFLOW:${CLI_NAME}: Deactivating Backup ${A_BKP_ID_DB} for client: .... "

      LO_MNT=$(mount -lt ext2,ext4 | grep -w "$LOOP_DEVICE" | awk '{ print $3 }'| grep -w "${STORDIR}/${CLI_NAME}/${CLI_CFG}")
      if [ -n "$LO_MNT" ]; then
...
if [[ ${ENABLE} == 'yes' ]]; then
   local A_BKP_ID_DB_OLD=$(get_active_cli_bkp_from_db $CLI_NAME $CLI_CFG)
